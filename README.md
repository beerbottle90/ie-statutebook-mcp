...
**Ireland 🇮🇪 legal research over the Model Context Protocol.**
irishstatutebook.ie — official Oireachtas publication
...
Clean server-rendered ELI documents down to section level, but **no search endpoint** (`/search` and `/searchresults.html` both 404). This server indexes whole Act bodies locally, so a phrase inside a section is findable.
...
For cross-language work — a Turkish question against a Ireland corpus — the
...
- No search endpoint and no year-index API — Act numbers are probed sequentially.
- Default texts are AS ENACTED: amendments are not applied.
- Acts are long (Companies Act 2014 = 1,448 sections) — prefer get_section.
...
```sh
docker build -t ie-statutebook-mcp .
docker run -p 8000:8000 -e PORT=8000 ie-statutebook-mcp
```
